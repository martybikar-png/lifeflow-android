...
internal fun errorActionHint(
    ...
    return when {
        showResetVaultAction ->
            "Use the reset path only for trust recovery, then rebuild access with a fresh authentication."

        showAuthenticateAction ->
            "Authentication is the safest first recovery step from this state."
        ...
        canGrantHealthPermissions(
            ...
            "Health access is still incomplete, so you can also repair the data path from Health Connect."

        else ->
            "Review settings first, then retry the recovery flow."
    }
}
